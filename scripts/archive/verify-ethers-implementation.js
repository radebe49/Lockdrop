    ...
    for (const method of expectedMethods) {
      if (contract[method]) {
        ...
        logTest(
          `Method: ${method}`,
          "fail",
          "Method not found in contract interface"
        );
      }
    }
    ...
    try {
      ...
      if (error.message.includes("MessageNotFound")) {
        logTest(
          "getMessage()",
          "pass",
          "Correctly throws MessageNotFound for non-existent message"
        );
      } else {
    ...
    try {
      ...
      if (eventFragment) {
        logTest(
          "MessageStored event",
          "pass",
          `Inputs: ${eventFragment.inputs.map((i) => i.name).join(", ")}`
        );
      } else {
    ...
    try {
      ...
      if (typeof countNumber === "number" && typeof countBigInt === "bigint") {
        logTest(
          "uint64 conversion",
          "pass",
          `Number: ${countNumber}, BigInt: ${countBigInt}`
        );
      } else {

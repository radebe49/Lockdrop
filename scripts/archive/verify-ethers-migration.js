  ...
  try {
    ...
    try {
      ...
      if (missingMethods.length === 0) {
        logTest(
          "ABI method availability",
          true,
          `All ${expectedMethods.length} methods found`
        );
      } else {
        logTest(
          "ABI method availability",
    ...
    try {
      ...
      if (messages.length > 0) {
        ...
        console.log(
          `     - unlockTimestamp: ${msg.unlockTimestamp ? "✓" : "✗"}`
        );
    ...
    try {
      ...
      if (availableErrors.length === expectedErrors.length) {
        logTest(
          ...
          true,
          `All ${expectedErrors.length} errors defined`
        );
      } else {
        const missing = expectedErrors.filter(
          (e) => !availableErrors.includes(e)
        );
        logTest(
          "Custom error interfaces",
          false,
    ...
    try {
      ...
      if (isValidBigInt && isValidAddress) {
        logTest(
          "Type compatibility",
          true,
          "BigInt and address types work correctly"
        );
      } else {
